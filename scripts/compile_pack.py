#!/usr/bin/env python
"""compile_pack.py — 从 Capability Bundle 确定性编译 compact pack。
...
用法: python scripts/compile_pack.py --bundle <dir> --out <dir> [--allow-over-budget]
"""
...
def build_promoted_skill_md(cap: dict, bundle: dict, card_text: str) -> str:
    ...
    lines = [
        ...
        *[f"  {line}" for line in description.strip().splitlines()],
        ...
        f"  distill.capability-revision: {cap['revision']}",
        f"  distill.bundle-id: {bundle['bundle_id']}",
        f"  distill.source-title: {bundle['book']['title']}",
    ]
    if tags:
        lines.append(f"  distill.tags: {', '.join(tags)}")
    lines += ["---", ""]
