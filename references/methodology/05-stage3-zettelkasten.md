...
5. 把 `candidates/glossary.md` 整理提升为 `books/<slug>/GLOSSARY.md`,并复制到
   `.distill/capabilities/book/glossary.md`（编译时随产物分发）
6. 路由表 / 能力索引（capability-index.md）由 `distill.py compile` 从 Bundle 自动生成,
   **不再手写 INDEX.md**;legacy-pack 流程仍可用 `references/templates/INDEX.md.template`
