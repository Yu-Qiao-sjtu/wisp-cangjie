#!/usr/bin/env python
"""build_chunks.py — 原生 Markdown/TXT → SourceDocument + 结构感知块（纯确定性）。

产出（写入 <sidecar>/，默认 books/<slug>/.distill/）：
...
用法: python scripts/build_chunks.py <source.md> --sidecar <dir> [--source-id src-main-book] [--max-chars 4000]
"""
...
def main() -> int:
    ...
    ap.add_argument("--sidecar", required=True, help="侧车目录，如 books/<slug>/.distill")
