#!/usr/bin/env python
"""apply_skill_patch.py — 最小补丁事务：快照 → 覆盖补丁文件 → 校验 → 可回滚。

repair/update 的落盘环节。补丁是"文件覆盖层"（patch 目录里的文件按相对路径覆盖目标），
不做自由重写；应用前自动快照，校验失败自动回滚。

用法:
  python scripts/apply_skill_patch.py apply --target <skill-dir> --patch-dir <dir> --snapshots <dir>
  python scripts/apply_skill_patch.py restore --target <skill-dir> --snapshot <snapshot-dir>
"""
...
from distill_common import WriterLock, snapshot_dir  # noqa: E402
