...
- **晋级能力（promoted）**: 按本文全部要求测触发精度,诱饵中必须含"应由来源路由入口处理"的场景
  （晋级 Skill 与路由入口互斥的近邻负例）;
- **路由能力（router）**: 测"经来源路由入口可达"——给定意图 prompt,路由表应指向正确的能力卡;
- **两类共同要求**: 实际完成代表任务并核对交付物；触发或可达通过不能替代执行通过。
- 评测用例格式见 `assets/schemas/eval-suite.schema.json`,可用 `scripts/run_trigger_evals.py` 做机械判分与
...
```sh
python scripts/run_output_evals.py prepare suite.json --out /tmp/distill-eval-run
# 每个任务在 <case_id>/<label>/ 中执行，回答写 output.md，交付物留在同目录。
python scripts/run_output_evals.py score suite.json --outputs /tmp/distill-eval-run --mapping /tmp/distill-eval-run/mapping.json --out report.md
```
...
- `<skill-dir>/test-prompts.json` — 回归评测用例
- `<skill-dir>/test-results.md` — 本次测试的通过率和失败分析 (审计用)

## 下一步

所有 skill 全部通过后,进入阶段 5 (交付),见 `07-stage5-deliver.md`: 生成面向读者的 DIGEST.md 精华长文,并把 skill 安装到用户的 skills 目录。
