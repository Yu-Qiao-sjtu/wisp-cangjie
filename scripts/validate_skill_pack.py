#!/usr/bin/env python
"""validate_skill_pack.py — 纯确定性 Skill 包静态校验（不调用模型）。
...
4. 全部 .md 文件中的相对引用（markdown 链接与裸 references|scripts|assets/... 路径）必须存在。
   解析基准与 wisp store 一致：最近一层含 SKILL.md 的目录（Skill 包根），不是 md 文件所在目录；
5. 输出每个 Skill 的行数/字节数概览。

用法: python scripts/validate_skill_pack.py <dir> [<dir> ...]
...
BARE_REF_RE = re.compile(
    r"(?<![\w/(])((?:references|scripts|assets)/[\w\-./]+\.[a-zA-Z0-9]+)")
...
def skill_base(md_path: Path, root: Path) -> Path:
    """引用解析基准：最近一层含 SKILL.md 的目录（Skill 包根），否则退回 md 所在目录。"""
    for cand in [md_path.parent, *md_path.parent.parents]:
...
def check_links(md_path: Path, text: str, base: Path) -> None:
    seen: set[str] = set()
    ...
    for root_arg in argv[1:]:
        ...
        for md in sorted(root.rglob("*.md")):
            ...
            try:
                ...
                continue
            check_links(md, text, skill_base(md, root))
            if md.name == "SKILL.md":
                skill_count += 1
                check_skill(md)
                lines = text.count("\n") + 1
