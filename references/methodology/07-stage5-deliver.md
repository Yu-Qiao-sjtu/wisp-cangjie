...
python scripts/distill.py compile \
  --bundle books/<slug>/.distill/capabilities \
...
- 每个核心方法论写一小节: 它解决什么问题 → 核心逻辑 → 原书示例（没有时如实说明，可用明确标记的合成演练）→ 什么时候会失效
...
模板: `references/templates/DIGEST.md.template`,输出到 `books/<slug>/DIGEST.md`。
...
**安装前必须执行脱敏清单**（交付物与 `books/<slug>/` 审计轨迹的唯一分界）:

1. 全文检索来源标识: 书名、作者名、人物昵称（如"某老师"）、出版社/年份、章节标题原文、逐字引文 — 命中即替换为改述或泛称;
2. 分数+期刊组合（如"9.412 分 PNAS"）降档为档位描述（"某 9 分文章"）;
3. frontmatter 的 `source_book`/`source_chapter` 字段填脱敏代号（"来源已脱敏"/"因果审读章"）或整字段移除;
4. 替换后重新编译并 grep 复核零残留,再执行安装;带完整溯源信息的产物只留在 `books/<slug>/` 内。

编译输出目录（如 `dist/<slug>/`）只是构建产物,宿主 (Wisp / Claude Code / Cursor 等) 不会从这里加载 skill。必须安装:

1. **问用户装哪里** (一次性问清,不要逐个 skill 问):
   - 用户级: `~/.wisp/skills/` (所有项目可用)
   - 项目级: 宿主的项目级 skills 目录 (如 `.claude/skills/`、`.cursor/skills/`)
   - 用户也可能只想要仓库形式 (发布到 GitHub),那就跳过安装
2. **single 模式**: 复制整个入口目录（含 `references/` 和已声明的 `resources/`）,1 个可发现入口;
