...
产出: `books/<slug>/BOOK_OVERVIEW.md` (按 `references/templates/BOOK_OVERVIEW.md.template` 填充)。
...
- **哪些内容可以 skill 化?** — 框架 / 清单 / 原则 / 决策程序 / 操作流程 / 计算规则 / 故障排查
- **哪些内容不适合 skill 化?** — 纯史料 / 纯故事 / 纯情感 (但可作为其他 skill 的 example)
- **预估 skill 数量**: 给一个粗略区间 (不要硬凑)
- **预估优先级**: 从"最能赋能普通人"的角度给候选 skill 排序

在 `BOOK_OVERVIEW.md` 建立**原书关键任务清单**：task_id、读者要完成的任务、原文位置、预期交付物、重要性及依据。以目录、正文、图表和用户目的为依据，不从旧版 verified 反推清单。内容类型按候选区分，混合型书籍不套单一门槛。
对原文没有提供的任务条件记缺口，不用常识补成作者的方法。此清单是阶段 1.5 `coverage-audit.md` 的独立核查基准。

## 质量门 (进入阶段 1 之前必须满足)

- [ ] 主旨能用一句话说清
- [ ] 骨架列出 3–7 个一级论点
- [ ] 关键术语按原文实际数量列出，不为数量凑概念
- [ ] 已检查局限、前提和未证明假设；无证据时标记待核查，不编造三条批判
- [ ] 已列出原书关键任务、来源位置、交付物及重要性依据
