#!/usr/bin/env python
"""impact_analysis.py — 依赖图构建 + change-set 影响范围分析（纯确定性）。
...
用法:
  python scripts/impact_analysis.py build --bundle <dir> [--chunks <chunks.jsonl>] --out <graph.json>
  python scripts/impact_analysis.py analyze --graph <graph.json> --change-set <cs.json> --out <report.md>
"""
...
from distill_common import TOOL_VERSION, dump_json, load_json, load_yaml  # noqa: E402
...
def analyze(graph: dict, change_set: dict) -> str:
    ...
    lines.append("\n> 未受影响的能力/入口不重编译，文件哈希保持不变（增量验收要求）。")
    return "\n".join(lines) + "\n"
