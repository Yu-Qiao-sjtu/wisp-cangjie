#!/usr/bin/env python
"""run_trigger_evals.py — 触发评测的确定性环节。

CLI 不能替宿主跑模型；本脚本负责三件确定性的事：
  ...
  score    对照 suite 判分：precision/recall/F1、兄弟混淆率、逐条配对结果
           （不做统计非劣声明——那需要预注册界值与配对检验）

results.jsonl 每行: {"case_id": "...", "run": 1, "selected_skill": "<slug>|none"}

用法:
  python scripts/run_trigger_evals.py split   <suite.json>
  python scripts/run_trigger_evals.py prepare <suite.json> --skills <slug1,slug2,...> --out <dir> [--set train|validation|all]
  python scripts/run_trigger_evals.py score   <suite.json> --results <results.jsonl> --out <report.md>
"""
...
from distill_common import dump_json, load_json  # noqa: E402
...
def cmd_score(suite_path: Path, results_path: Path, out_path: Path) -> int:
    ...
    report = (f"# 触发评测判分 — {target}\n\n"
              ...
              + "\n".join(rows)
              + "\n\n> 本报告只给原始配对计数与比率；统计非劣需预注册界值 + McNemar/配对 Bootstrap，不在此自动宣布。\n")
