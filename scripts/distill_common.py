#!/usr/bin/env python
"""distill_common.py — 蒸馏确定性脚本的共享工具（纯本地、不调模型）。

提供：frontmatter 解析、哈希、确定性缓存键、per-run workdir、writer lock、
staging + 原子发布、发布哈希登记与本地手改检测。
"""
...
TOOL_VERSION = "wisp-cangjie v1.0.0"
...
class WriterLock:
    ...
    def __init__(self, target: Path):
        self.lock_path = target.with_name(target.name + ".distill-lock")
